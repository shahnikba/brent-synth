...
`diagnostics.run_all` on 4753 daily log returns, 2007-07-30 to 2026-09-03.
Rerun after the Hill guard and the GPD significance gate: the Hill figures
are unchanged, and the gate withholds the gain tail's index (see below).*
...
underflows (1.9e−213). Volatility is forecastable and stays forecastable,
...
half the effective power-law index of the gain tail (1/ξ ≈ 4.7 on losses; on gains ξ is not
distinguishable from zero at 95% — ξ/se = 1.25 — so no index is quoted).
The mean-excess function of losses rises with the threshold — slope +0.27
over the top fifth of thresholds, mean excess climbing from 0.021 to 0.038
— which is the GPD signature rather than the flat profile an exponential
tail would give, consistent with a genuinely heavy left
